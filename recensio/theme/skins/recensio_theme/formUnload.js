/* This script as of plone4.0.2 does not work well with IE8. See the XXX
 * Notes for changes */
...
if (!window.beforeunload)
    (function ($) {
        ...
        Class.isAnyFormChanged = function () {
            // XXX Our little plone modification
            // If we don't declare form as a local variable, form becomes
            // a global one. This has been fixed in a similar manner in plone
            // but not yet in 4.0x
            // In ie8 the issue was that form existed already and assingment
            // was not possible
            var form;
            for (var i = 0; (form = this.forms[i++]); ) {
                if (this.isElementChanged(form)) return true;
            }
            return false;
        };
        ...
        Class.addForms = function () {
            ...
            $.each(arguments, function () {
                if (this.tagName.toLowerCase() == "form") self.addForm(this);
                else self.addForms.apply(self, $(this).find("form").get());
            });
        };
        Class.removeForms = function () {
            ...
            $.each(arguments, function () {
                ...
                } else self.removeForms.apply(self, $(this).find("form").get());
            });
        };
        ...
        Class.isElementChanged = function (ele) {
            ...
            if (!method && ele.type && ele.name) method = this.chkType[ele.type];
