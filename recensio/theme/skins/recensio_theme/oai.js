...
function showResults(data) {
    ...
    for (var result_id = data.length - 1; result_id >= 0; result_id--) {
        ...
        for (var i = 0; i < result["keywords"].length; i++) {
            tmpl.find(".oai_keywords .value ul").append(
                "<li>" + result["keywords"][i] + "</li>"
            );
        }
        var subtitle = tmpl.find(".oai_subtitle td.value");
        if (subtitle.text()) {
            subtitle.text(
                subtitle.text()[0].toUpperCase() + subtitle.text().substring(1)
            );
        }
        for (var i = 0; i < result["authors"].length; i++) {
            var author = result["authors"][i];
            tmpl.find(".oai_authors .value").append(
                '<div class="author"><span class="firstname">' +
                    author["firstname"] +
        ...
        for (var i = 0; i < result["editors"].length; i++) {
            ...
            tmpl.find(".oai_editors .value").append(
                '<div class="editor"><span class="firstname">' +
                    editor["firstname"] +
                    '</span> <span class="lastname">' +
                    editor["lastname"] +
                    "</span></div>"
        ...
        for (var i = 0; i < ddcs.length; i++) {
            ...
            jq(result[ddc]).each(function (i, data) {
                ...
                var matching_elems = jq("#" + source_id + " option").filter(function () {
                ...
                if (ddc == "ddcPlace") {
                    var matching_elems = jq("#" + ddc + " option").filter(function () {
                        return this.text == li_text;
                    });
                    if (matching_elems.length) {
...
function takeOver(elem) {
    ...
    j.find(".author").each(function () {
        ...
        jq("#archetypes-fieldname-authors tr#datagridwidget-row").each(function () {
            ...
            if (
                new_firstname == existing_firstname &&
                new_lastname == existing_lastname
            ) {
                name_exists = true;
            }
        });
        if (!name_exists) {
            ...
            jq(
                "#archetypes-fieldname-authors #datagridwidget-row:last #firstname_authors_new"
            ).val(new_firstname);
            jq(
                "#archetypes-fieldname-authors #datagridwidget-row:last #lastname_authors_new"
            ).val(new_lastname);
        }
    });
    var ddcs = ["ddcSubject", "ddcTime", "ddcPlace"];
        ...
        j.find(".oai_" + ddc + " li").each(function () {
            if (jq(this).attr("class") == "missing") {
                if (!missing_flag) {
                    ...
                    jq("#" + ddc + "_help").append(
                        jq("#missing_explanation_2_template").clone().show()
                    );
                    missing_flag = true;
                }
