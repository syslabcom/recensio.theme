...
class AuthorSearchView(BrowserView):
    ...
    def authors(self):
        ...
        if authors:
            ...
            authors = [dict(name=x, reviews=reviews.get(safe_unicode(x), 0), presentations=presentations.get(safe_unicode(x), 0), comments=comments.get(safe_unicode(x), 0)) for x in
                catalog.uniqueValuesFor('authors') if True in
                [y in x.lower() for y in items]]
        else:
            authors = [dict(name=x, reviews=reviews.get(safe_unicode(x), 0), presentations=presentations.get(safe_unicode(x), 0), comments=comments.get(safe_unicode(x), 0)) for x in
                catalog.uniqueValuesFor('authors')]
        authors = filter(lambda x: x['presentations'] + x['reviews'] + x['comments'] != 0, authors)
    ...
    def alpha_index(self):
        ...
        for letter in self.ALPHABET:
            part = filter(lambda a: a['name'].startswith(letter), self.authors)
            if part:
