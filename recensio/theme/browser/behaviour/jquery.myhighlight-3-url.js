...
Modified by Manuel Reinhardt to insert anchors instead of spans, 2011-9-8

Highlights arbitrary terms. In this version only really makes sense when using URL patterns.
    ...
    function innerHighlight(node, pattern) {
        ...
        if (node.nodeType === 3) { // 3 - Text node
            ...
            if (pos >= 0 && node.data.length > 0) { // .* matching "" causes infinite loop
                ...
                spanNode.href = middleClone.nodeValue; // set href
                // parentNode ie. node, now has 3 nodes by 2 splitText()s, replace the middle with the highlighted spanNode:
                middleBit.parentNode.replaceChild(spanNode, middleBit); 
                skip = 1; // skip this middleBit, but still need to check endBit
            }
        } else if (node.nodeType === 1 && node.childNodes && !/(script|style|a)/i.test(node.tagName)) { // 1 - Element node
