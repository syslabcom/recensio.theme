...
class TestBreadcrumbs(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_volume_id = self.pub.invokeFactory("Volume", id="test-volume")
        ...
        newid = self.test_issue.invokeFactory("Review Monograph", id="test-review")
        ...
        from Products.CMFPlone.browser.navigation import (
            RootPhysicalNavigationBreadcrumbs,
        )
        ...
        sm.registerAdapter(
            factory=RootPhysicalNavigationBreadcrumbs,
    ...
    def test_breadcrumbs_for_published_publication(self):
        breadcrumbs_view = getMultiAdapter(
            (self.test_review, self.portal.REQUEST), name="breadcrumbs_view"
        )
        ...
        urls = [
            cr["absolute_url"].replace("rezensionen", "rezensionen/zeitschriften")
            for cr in crumbs
        ]
