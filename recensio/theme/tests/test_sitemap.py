...
class TestSiteMap(unittest.TestCase):
    ...
    def test_sitemap_priorities(self):
        ...
        sitemap_view = getMultiAdapter((self.portal, self.portal.REQUEST),
                                       name='sitemap.xml.gz')
        ...
        high_prio_priorities = set([i.get("priority", None)
                                    for i in unprioritised_obs])

        self.assertEquals(
            low_prio_priorities, set([0.10000000000000001]),
            msg = u"All low priority items have a priority of 0.1")
        self.assertEquals(
            high_prio_priorities, set([None]),
            msg = (u"Items which do not have a low priority do not have "
